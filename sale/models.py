...
class SaleOrder(generic.BO):
    ...
    partner = models.ForeignKey(Partner, on_delete = models.CASCADE,verbose_name=_(
        "partner"), limit_choices_to={"partner_type": "C"})
...
class PaymentCollection(generic.BO):
    ...
    so = models.ForeignKey(
        SaleOrder, on_delete=models.CASCADE, verbose_name=_("sale order"))
    partner = models.ForeignKey(Partner, on_delete=models.CASCADE, verbose_name=_(
        "partner"), blank=True, null=True)
    ...
    bank = models.ForeignKey(BankAccount, on_delete=models.CASCADE, verbose_name=_(
...
class OfferSheet(generic.BO):
    ...
    attach = models.FileField(_('offer sheet file'),blank=True,null=True,upload_to='offer sheet',help_text='您可导入报价明细，模板请参考文档FD0006')
    status = models.BooleanField(_("executed"),default=0)
    event_time = models.DateTimeField(_("event time"),blank=True,null=True)
