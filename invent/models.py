...
class Inventory(generic.BO):
    ...
    """
    index_weight = 1
    org = models.ForeignKey(Organization, on_delete=models.CASCADE, verbose_name=_(
    ...
    material = models.ForeignKey(
        Material, on_delete=models.CASCADE, verbose_name=_("material"))
    measure = models.ForeignKey(
        Measure, on_delete=models.CASCADE, verbose_name=_("measure"))
...
class InitialInventory(generic.BO):
    ...
    attach = models.FileField(_('attach'),blank=True,null=True,upload_to='inventory',help_text='参考FD0002模板文档')
...
class StockIn(generic.BO):
    ...
    warehouse = models.ForeignKey(
        Warehouse, on_delete=models.CASCADE, verbose_name=_("warehouse"))
    title = models.CharField(_("title"),max_length=const.DB_CHAR_NAME_40)
    user = models.ForeignKey(User, on_delete=models.CASCADE, verbose_name=_(
        "user"), blank=True, null=True)
    status = models.CharField(_("status"),max_length=const.DB_CHAR_CODE_2,default='0',choices=STATUS)
    execute_time = models.DateTimeField(_("execute time"),blank=True,null=True)
    po = models.ForeignKey(PurchaseOrder, on_delete=models.CASCADE, verbose_name=_(
        "purchase order"), null=True, limit_choices_to={"entry_status": "0"}, blank=True)
    ...
    def action_entry(self,request):
        ...
        执行入库操作
        """
        print("action_entry--------initem_set.count() %d" % self.initem_set.count())
...
class StockOut(generic.BO):
    ...
    title = models.CharField(_("title"),max_length=const.DB_CHAR_NAME_40)
    project = models.ForeignKey(Project, on_delete=models.CASCADE, verbose_name=_("project"),blank=True,null=True)
    wo = models.ForeignKey(WorkOrder,on_delete=models.CASCADE, verbose_name=_("work order"),blank=True,null=True)
    description = models.TextField(_("description"),blank=True,null=True)
    amount = models.DecimalField(_("money of amount"),max_digits=14,decimal_places=4,blank=True,null=True)
    user = models.ForeignKey(User, on_delete=models.CASCADE, verbose_name=_(
        "out user"), blank=True, null=True)
    status = models.CharField(_("status"),max_length=const.DB_CHAR_CODE_2,default='0',choices=STATUS)
    execute_time = models.DateTimeField(_("execute time"),blank=True,null=True)
...
class WareReturn(generic.BO):
    ...
    out = models.ForeignKey(
        StockOut, on_delete=models.CASCADE, verbose_name=_('StockOut'))
    amount = models.DecimalField(_("money of amount"),max_digits=14,decimal_places=4,blank=True,null=True)
    user = models.ForeignKey(User, on_delete=models.CASCADE, verbose_name=_(
        "out user"), blank=True, null=True)
    status = models.CharField(_("status"),max_length=const.DB_CHAR_CODE_2,default='0',choices=STATUS)
...
class InItem(InOutDetail):
    ...
    po_item = models.ForeignKey(POItem, on_delete=models.CASCADE,verbose_name=_(
        "po item"), blank=True, null=True)
