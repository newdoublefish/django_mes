...
class Modal(models.Model):
    ...
    content_type = models.ForeignKey(ContentType, verbose_name=_(
        "content type"), on_delete=models.DO_NOTHING, limit_choices_to={"app_label__in": ['basedata', 'organ']})
...
class Node(models.Model):
    ...
    handler = models.TextField(_("handler"),blank=True,null=True,help_text='自定义SQL语句，优先高于指定用户、岗位、角色')
    # added by zhugl 2015-05-10
...
class Instance(models.Model):
    ...
    starter = models.ForeignKey(User, on_delete=models.DO_NOTHING, verbose_name=_(
        "start user"), related_name="starter")
...
class History(models.Model):
    ...
    inst = models.ForeignKey(
        Instance, on_delete=models.DO_NOTHING, verbose_name=_("workflow instance"))
    node = models.ForeignKey(Node, on_delete=models.DO_NOTHING,verbose_name=_(
        "workflow node"), blank=True, null=True)
    user = models.ForeignKey(
        User, on_delete=models.DO_NOTHING, verbose_name=_("submitter"))
    ...
    def get_action_desc(self):
        action_mapping = {0:'提交',1:'同意',3:'拒绝',4:'终止',}
...
class TodoList(models.Model):
    ...
    model_name = models.CharField(_("model name"),max_length=const.DB_CHAR_NAME_60,blank=True,null=True)
    user = models.ForeignKey(
        User, on_delete=models.DO_NOTHING, verbose_name=_("handler"))
    ...
    def modal_dsc(self):
        return '%s'%(self.inst.modal.name)
    modal_dsc.short_description = '业务流程'
    ...
    def submitter(self):
        if self.inst.starter.last_name or self.inst.starter.first_name:
            return "%s%s"%(self.inst.starter.last_name,self.inst.starter.first_name)
        return "%s"%(self.inst.starter.username)
